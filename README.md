...
Kestra wasn’t able to upload an entire folder to GCS or use flexible file selection methods such as wildcards (`*`) or Python’s `glob` function. If multiple CSVs could have been uploaded directly, the full ELT process — including transformation with dbt — could have been handled entirely within Kestra. This limitation also led to coalescing the Spark output into one Parquet file/year in GCS to simplify the upload process into BigQuery using Kestra, which totally sacrificed efficiency. Kestra being a relatively new tool also meant limited online documentation and community support.

- **Spark-to-BigQuery Connector Issue**

Attempts were made to load data directly from Spark into BigQuery using the Spark–BigQuery connector, but it was not successfully connected, data had to be written to GCS first using spark, and then loaded into BigQuery using Kestra. It made the ETL process less efficient than a direct Spark-to-BigQuery integration.


- **Limited Orchestration and Containerization**

The pipeline is not fully orchestrated from end to end. Ideally, a single trigger should initiate the entire workflow without manual steps. While Kestra was used to orchestrate part of the flow, the initial Spark script could have been containerized and built into a Docker image and executed within the same docker-compose environment as Kestra. In addition, the Parquet file generated by Spark and loaded to GCS had to be manually renamed before it could be loaded into BigQuery via Kestra, since Spark does not support directly specifying output file names and a workaround could not be implemented.
